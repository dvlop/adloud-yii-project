...
?>
    ...
    <div class="col-sm-12">
        <div class="col-sm-12 statistic_menu">
            ...
            <div class="col-sm-3">
                <div class="row">
                    <select id="type" name="graph_type" class="graph_settings graph_type">
                        <option value="sites" <?= $type == 'sites' ? 'selected=true' : ''?> value="По кампаниям"><?=Yii::t('webmaster_stats','По площадкам')?></option>
                        <option value="date"  <?= $type == 'date' ? 'selected=true' : ''?>  value="По объявлениям"><?=Yii::t('webmaster_stats','По датам')?></option>
                    </select>
                </div>
            </div>
            <div class="col-sm-3">
                ...
                <div class="row">
                    <select id="day-period" name="graph_time" class="graph_settings graph_time">
                        <option value="actual" <?= $status == 'actual' ? 'selected=true' : ''?>><?=Yii::t('webmaster_stats','Актуальные')?></option>
                        <option value="archived"  <?= $status == 'archived' ? 'selected=true' : ''?>><?=Yii::t('webmaster_stats','Архивные')?></option>
                    </select>
                </div>
                <?php endif;?>
            </div>
            ...
            <div class="col-sm-3 set_period">
                <p><?=Yii::t('webmaster_stats','Установить период');?> <span class="caret right"></span></p>
            </div>
    ...
    <div class="col-sm-12">
        <?php if(count($chartData['label']) > 1):?>
            ...
            <button class="toggle-chart show" type="button"><?=Yii::t('webmaster_stats','Скрыть график')?></button>
            <button class="toggle-chart hide" type="button"><?=Yii::t('webmaster_stats','Показать график')?></button>
    ...
    <div class="col-sm-12">
        <?php if(isset($stats)):?>
            <table class="table table-striped table-hover adloud_table statistic_table">
                <thead>
                <tr>
                    <th class="col-sm-3"><?=Yii::t('webmaster_stats','Описание')?></th>
                    <th class="col-sm-3"><?=Yii::t('webmaster_stats','Показы')?></th>
                    <th class="col-sm-2"><?=Yii::t('webmaster_stats','Клики')?></th>
                    <th class="col-sm-2">CTR</th>
                    <th class="col-sm-2"><?=Yii::t('webmaster_stats','Доход')?></th>
                </tr>
                </thead>
                <tbody>
                <?php if($type == 'date'):?>
                ...
                <?php endif; ?>
                <tr>
                    <td><?=Yii::t('webmaster_stats','Всего')?></td>
                    <td><?=$total['shows']?></td>
                    <td><?=$total['clicks']?></td>
