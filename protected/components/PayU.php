...
class PayU
{
    var $luUrl = "https://secure.payu.ua/order/lu.php",
        $button = "<input id='paymentSubmit' type='submit'>",
        $debug = 0,
        $showinputs = "hidden";
    ...
    private $LUcell
        = array('MERCHANT'       => 1, 'ORDER_REF' => 0, 'ORDER_DATE' => 1, 'ORDER_PNAME' => 1, 'ORDER_PGROUP' => 0,
...
#-----------------------------	
    private function makeString($name, $val)
    {
        $str = "";
        if (!is_array($val)) {
            return '<input type="' . $this->showinputs . '" name="' . $name . '" value="' . $val . '">' . "\n";
        }
        foreach ($val as $v) {
